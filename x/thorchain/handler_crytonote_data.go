...
// CryptonoteDataHandler is to handle crytonote data message
...
// Run it the main entry point to execute crytonote data logic
func (h CryptonoteDataHandler) Run(ctx cosmos.Context, m cosmos.Msg) (*cosmos.Result, error) {
	msg, ok := m.(*MsgSetCryptonoteData)
	if !ok {
		return nil, errInvalidMessage
	}
	ctx.Logger().Info("receive crytonote data", "crytonote data", msg.CryptonoteData)
...
func (h CryptonoteDataHandler) handle(ctx cosmos.Context, msg MsgSetCryptonoteData) error {
	ctx.Logger().Info("handleMsgSetCryptonoteData request", "crytonote data", msg.CryptonoteData)
	return h.handleV75(ctx, msg)
}

func (h CryptonoteDataHandler) handleV75(ctx cosmos.Context, msg MsgSetCryptonoteData) error {
	nodeAccount, err := h.mgr.Keeper().GetNodeAccount(ctx, msg.Signer)
	if err != nil {
		ctx.Logger().Error("fail to get node account", "error", err, "address", msg.Signer.String())
